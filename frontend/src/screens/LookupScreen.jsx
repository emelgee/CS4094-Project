...
function PokemonDetail({ selected, detail, loading, onBack, onSelect }) {
                  ...
                  <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 0 }}>
                    {[base.ability1, base.ability2].filter(Boolean).map((a, i) => (
                      <span key={a} style={{ display: "flex", alignItems: "center" }}>
                        ...
                        <span style={{ fontSize: 15, fontWeight: 600, color: "#e4e6ef" }}>{fmtAbility(a)}</span>
                      </span>
                    ))}
                  </div>
                  {base.ability_hidden && (
                    <div style={{ marginTop: 8, fontSize: 13, color: "#5a6380" }}>
                      {fmtAbility(base.ability_hidden)} <em style={{ fontSize: 11 }}>(Hidden)</em>
                    </div>
                  )}
                </div>
              </div>
                        ...
                        <div style={{ borderTop: "1px solid #1a2030" }}>
                          {rows.map((r, i) => (
                            <div key={i} style={{
                              ...
                              <span style={{ color: "#c8cde0", textTransform: "capitalize", textAlign: "center" }}>
                                {(r.location_name || r.area_name || "").replace(/-/g, " ")}
                              </span>
                            </div>
                          ))}
                        </div>
            ...
            {moveTabs.length > 0 && (
              <div className="card" style={{ display: "flex", flexDirection: "column", maxHeight: 340, overflow: "hidden" }}>
                ...
                <div style={{ flex: 1, overflowY: "auto" }}>
                  {currentTab && (
                    <MoveTable moves={currentTab.moves} showLevel={currentTab.showLevel} />
                  )}
                </div>
              </div>
            )}
